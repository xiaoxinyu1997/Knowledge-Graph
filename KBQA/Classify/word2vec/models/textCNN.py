import torch
import torch.nn as nn
import torch.nn.functional as F
import numpy as np
from gensim.models import word2vec, KeyedVectors
from KBQA.Classify.word2vec.utils import get_embedding_matrix


class Config(object):

    """配置参数"""
    def __init__(self, dataset):
        self.model_name = 'TextCNN'
        self.train_path = dataset + '/train.txt'                                # 训练集
        self.dev_path = dataset + '/dev.txt'                                    # 验证集
        self.test_path = dataset + '/test.txt'                                  # 测试集
        self.class_list = [x.strip() for x in open(
            dataset + '/class.txt', encoding='utf-8').readlines()]              # 类别名单
        # self.vocab_path = dataset + '/data/vocab.pkl'                                # 词表
        self.save_path = './saved_model' + '/saved_dict/' + self.model_name + '.ckpt'        # 模型训练结果
        self.log_path = dataset + '/log/' + self.model_name

        self.dropout = 0.2                                              # 随机失活
        self.require_improvement = 1000                                 # 若超过1000batch效果还没提升，则提前结束训练
        self.num_classes = len(self.class_list)                         # 类别数
        self.multi_class = True
        self.n_vocab = 0                                                # 词表大小，在运行时赋值
        self.num_epochs = 20                                            # epoch数
        self.batch_size = 16                                           # mini-batch大小
        self.max_len = 30                                            # 每句话处理成的长度(短填长切)
        self.learning_rate = 1e-3                                       # 学习率
        self.embedding_pretrained = torch.tensor(get_embedding_matrix(self.max_len))   # 预训练词向量
        self.embed = self.embedding_pretrained.size(1)                # 字向量维度
        self.filter_sizes = (2, 3, 4)                                   # 卷积核尺寸
        self.num_filters = 3                                          # 卷积核数量(channels数)
        self.device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')   # 设备


'''Convolutional Neural Networks for Sentence Classification'''


class Model(nn.Module):
    def __init__(self, config):
        super(Model, self).__init__()
        if config.embedding_pretrained is not None:
            self.embedding = nn.Embedding.from_pretrained(config.embedding_pretrained, freeze=False)
        else:
            self.embedding = nn.Embedding(config.n_vocab, config.embed, padding_idx=config.n_vocab - 1)
        self.convs = nn.ModuleList(
            [nn.Conv2d(1, config.num_filters, (k, config.embed)) for k in config.filter_sizes])
        self.dropout = nn.Dropout(config.dropout)
        self.fc = nn.Linear(config.num_filters * len(config.filter_sizes), config.num_classes)
        self.sig = nn.Sigmoid()
        self.multi_class = config.multi_class

    def conv_and_pool(self, x, conv):
        x = F.relu(conv(x)).squeeze(3)
        x = F.max_pool1d(x, x.size(2), int(x.size(2)/2)).squeeze(2)
        return x

    def forward(self, x):
        # print(x)   # (batch, seq)
        out = self.embedding(x.to(torch.int64))  # (batch, seq, embed)
        # print(out.shape)
        out = out.unsqueeze(1).to(dtype=torch.float)  # (batch, 1, seq, embed)
        # print(out.shape)
        out = torch.cat([self.conv_and_pool(out, conv) for conv in self.convs], 1)  # (batch, num_filters * len(filter_size))
        out = self.dropout(out)
        out = self.fc(out)  # (batch, classes)
        if self.multi_class == True:
            out = self.sig(out)
        return out